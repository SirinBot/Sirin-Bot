...
function defaultConfig() {
  return {
    ...
    // Colour roles channel (dropdown menus)
    colourChannelId: "1272601527164600403",

    // Message IDs for each panel (can be blank, bot will create & store)
    messages: {
      ...
      // Colour menu message IDs (two menus)
      colourMenu1: "",
      colourMenu2: "",
    },
  };
...
// ---------------- COLOUR ROLES (DROPDOWN, 2 MENUS) ----------------
// NOTE: Discord select menus have a max of 25 options, so we split into 2.
const COLOUR_ROLES = [
  { name: "♰ ✦ Ash Veil ✦ ♰", roleId: "1271989191772999762" },
  { name: "♰ ✦ Obsidian ✦ ♰", roleId: "1271989807610069033" },
  { name: "♰ ✦ Cinder ✦ ♰", roleId: "1271990032512581685" },
  { name: "♰ ✦ Flicker ✦ ♰", roleId: "1271991428364370030" },
  { name: "♰ ✦ Hemogoblin ✦ ♰", roleId: "1271992162816626768" },
  { name: "♰ ✦ Crimson Shard ✦ ♰", roleId: "1271992592543780935" },
  { name: "♰ ✦ Vintner ✦ ♰", roleId: "1272002406368149524" },
  { name: "♰ ✦ Ember Kiss ✦ ♰", roleId: "1272002180622057583" },
  { name: "♰ ✦ Scorched ✦ ♰", roleId: "1272001915378470952" },
  { name: "♰ ✦ Corroded ✦ ♰", roleId: "1272001805290831902" },
  { name: "♰ ✦ Circuit ✦ ♰", roleId: "1309198526495981580" },
  { name: "♰ ✦ Embercore ✦ ♰", roleId: "1272001508841623593" },
  { name: "♰ ✦ Toxic Grove ✦ ♰", roleId: "1272001227051368540" },
  { name: "♰ ✦ Venom Lichen ✦ ♰", roleId: "1272001105160573001" },
  { name: "♰ ✦ Cyber Jade ✦ ♰", roleId: "1272000992082264136" },
  { name: "♰ ✦ Viridian Hex ✦ ♰", roleId: "1272000883621761047" },
  { name: "♰ ✦ Neon Rift ✦ ♰", roleId: "1271994645114781769" },
  { name: "♰ ✦ Abyssal ✦ ♰", roleId: "1271994508510236723" },
  { name: "♰ ✦ Cryo Byte ✦ ♰", roleId: "1271994090153574494" },
  { name: "♰ ✦ Byte Wraith ♰ ✦", roleId: "1271994238795780177" },
  { name: "♰ ✦ Nullshade ♰ ✦", roleId: "1271993606240206890" },
  { name: "♰ ✦ Darknet Fiend ♰ ✦", roleId: "1271993487591604346" },
  { name: "♰ ✦ Hellspire ♰ ✦", roleId: "1271993365302345770" },
  { name: "♰ ✦ Ether Pulse ♰ ✦", roleId: "1271993251271934022" },
  { name: "♰ ✦ Glitch Blossom ♰ ✦", roleId: "1271993777158098974" },
  { name: "♰ ✦ Aurora Wraith ♰ ✦", roleId: "1408855617975615631" },
  { name: "♰ ✦ Voltage Specter ♰ ✦", roleId: "1408856244378402887" },
  { name: "♰ ✦ Cyber Siren ♰ ✦", roleId: "1408858625560154132" },
];

const COLOUR_CHUNK_1 = COLOUR_ROLES.slice(0, 14);
const COLOUR_CHUNK_2 = COLOUR_ROLES.slice(14);
...
function findPanelByMessage(message) {
  // By messageId (preferred)
  const byId = PANELS.find((p) => cfg.messages?.[p.key] === message.id);
  if (byId) return byId;

  // Fallback: by title (survives reposts)
  const content = message.content || "";
  return PANELS.find((p) => content.startsWith(`**${p.title}**`)) || null;
}
...
function isTextChannel(ch) {
  // discord.js v14 safe check:
  return !!ch && typeof ch.send === "function" && typeof ch.messages?.fetch === "function";
}
...
async function ensureReactionPanel(channel, panel) {
  ...
  if (!msg) {
    ...
    if (msg.content !== desired) await msg.edit({ content: desired });
  }
  ...
  for (const item of panel.items) {
    try {
      ...
      console.log(`Could not react with emoji ${item.emojiId} (${panel.key}):`, e?.message ?? e);
    }
  }
...
function buildColourMenu(customId, placeholder, chunk) {
...
async function ensureColourMenus(channel) {
  const header = `**Pick your Colour**\nReact to choose your colour role.`;

  // MENU 1
  let msg1 = null;
  if (cfg.messages.colourMenu1) {
    try {
      msg1 = await channel.messages.fetch(cfg.messages.colourMenu1);
    } catch {
      msg1 = null;
  ...
  const row1 = buildColourMenu("colour_menu_1", "Choose your Colour! (1/2)", COLOUR_CHUNK_1);

  if (!msg1 || msg1.author?.id !== client.user.id) {
    msg1 = await channel.send({ content: header, components: [row1] });
    cfg.messages.colourMenu1 = msg1.id;
    saveConfig(cfg);
    console.log("Posted colour menu 1 ->", msg1.id);
  } else {
    await msg1.edit({ content: header, components: [row1] });
  }
  ...
  if (cfg.messages.colourMenu2) {
    try {
      msg2 = await channel.messages.fetch(cfg.messages.colourMenu2);
    } catch {
      msg2 = null;
  ...
  const row2 = buildColourMenu("colour_menu_2", "Choose your Colour! (2/2)", COLOUR_CHUNK_2);

  if (!msg2 || msg2.author?.id !== client.user.id) {
    msg2 = await channel.send({ components: [row2] });
    cfg.messages.colourMenu2 = msg2.id;
    saveConfig(cfg);
    console.log("Posted colour menu 2 ->", msg2.id);
  } else {
    await msg2.edit({ components: [row2] });
  }
}
...
async function refreshAllPanels() {
  ...
  if (!isTextChannel(rrChannel)) throw new Error("RR channel invalid");

  // Reaction panels in order (Age first)
  for (const panel of PANELS) {
    await ensureReactionPanel(rrChannel, panel);
  }

  // Colour channel menus
  const colourChannel = await guild.channels.fetch(cfg.colourChannelId);
  if (!isTextChannel(colourChannel)) throw new Error("Colour channel invalid");
...
async function handleReactionToggle(reaction, user, adding) {
  ...
  const panel = findPanelByMessage(reaction.message);
  if (!panel) return;
...
client.on(Events.MessageReactionAdd, (reaction, user) => {
  handleReactionToggle(reaction, user, true).catch((e) => console.log("ReactionAdd error:", e));
});

client.on(Events.MessageReactionRemove, (reaction, user) => {
  handleReactionToggle(reaction, user, false).catch((e) => console.log("ReactionRemove error:", e));
});

// ---------------- COLOUR MENU HANDLER ----------------
function allColourRoleIds() {
  return new Set(COLOUR_ROLES.map((c) => c.roleId));
}
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  const chosenRole = guild.roles.cache.get(roleId);
  if (!chosenRole) {
    return interaction.reply({ content: "That colour role no longer exists.", ephemeral: true });
  }

  // Remove any other colour roles first (exclusive colour role)
  const memberRoles = member.roles.cache;
  const colourIds = allColourRoleIds();

  for (const r of memberRoles.values()) {
    if (colourIds.has(r.id) && r.id !== roleId) {
  ...
  await interaction.reply({ content: `Colour set to **${chosenRole.name}** ✅`, ephemeral: true });
});
